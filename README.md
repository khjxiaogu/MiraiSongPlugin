...
- 由于网易云音乐最新限制，网易云音乐API需要登录cookie才能正常使用，请在配置项中找到neteasecookies并[设置正确cookie](#网易cookie获取)。 
# 最新公告
- XML卡片在群聊中会变成链接，经查是被阻挡了。
- Share卡片在群聊中无法发送，同上。
- LightApp已经完全改成Mirai，旧版本LightApp卡片无法继续发送。
- 百度音乐（千千）API暂时失效，请等待修复。
- 网易云音乐api需要登录cookie才能访问[设置正确cookie](#网易cookie获取)。
- 目前只维护网易和QQ音乐，酷狗，千千，喜马拉雅由于使用较少不再维护。
# 声明
