...
public class KugouMusicSource implements MusicSource {
	...
	public MusicInfo get(String keyword) throws Exception {
		...
		return new MusicInfo(
				info.get("song_name").getAsString(),
				info.get("author_name").getAsString(),
				info.get("img").getAsString(),
				info.get("play_url").getAsString(),
				"https://www.kugou.com/song/#hash="+song+"&album_id="+info.get("album_id").getAsString(),
				"酷狗");
	}
